...
class Child1 extends Component{
  ...
  componentDidUpdate(){
    var stateData=this.state.drop;
    var data=this.props.data1.filter(item => item.category === stateData);
    ...
    var tooltip = d3.select("body")
        ...
        .on("mouseover",(event,d)=>{
          tooltip.html(d.x.toString().concat(",",d.y)).style("visibility", "visible").style("background", "white").style("border-color", "black")
        })
        .on("mousemove", (event) =>{
